...
function diffToOps(prev: string, next: string): Op[] {
    ...
    if (deleted.length && inserted.length) {
        // replace
        return [
            { op: 'replace', pos: start, len: deleted.length, text: inserted },
        ]
...
export default function Note({ noteId }: { noteId: string }) {
    ...
    useEffect(() => {
        ...
        function onRemoteEdit(payload: {
            ...
            prevContentRef.current = applyOps(
    ...
    function onChange(e: React.ChangeEvent<HTMLTextAreaElement>) {
        ...
        batchTimerRef.current = window.setTimeout(() => {
            if (
                !outboundRef.current ||
                outboundRef.current.ops.length === 0 ||
                !socket
            )
                return
            ...
            <main className="flex flex-1 flex-col lg:flex-row h-full">
                <div className="flex flex-1 flex-col p-4 md:p-8 h-full">
                    ...
                    <div className="relative flex flex-1 flex-col lg:h-auto h-full">
                        <div className="relative flex flex-1 flex-col rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900/50 h-full">
                            <textarea
                ...
                <aside className="hidden w-full max-w-sm shrink-0 border-l border-gray-200 dark:border-gray-800 bg-white dark:bg-background-dark p-6 lg:flex lg:flex-col">
                    <div className="flex flex-col gap-8">
                        <div>
                            <h3 className="text-[#1F2937] dark:text-white text-lg font-bold mb-3">
                                Activity Stream
                            </h3>
                            <ul className="space-y-4">
                                <li className="flex items-center gap-3">
                                    <div>
                                        ...
                                        <p className="text-xs text-[#6B7281] dark:text-gray-400">
