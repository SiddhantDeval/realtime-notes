...
export default function AuthCallback() {
  ...
  useEffect(() => {
    ...
    if (token) {
        ...
        const tempSession = { token, user: { id: "", email: "", full_name: "" } };
        localStorage.setItem(localStorageAuthKey, JSON.stringify(tempSession));
        ...
        localStorage.setItem(localStorageAuthKey, JSON.stringify({ token, user: { id: "loading...", email: "...", full_name: "Loading..." } }));
        
        // This event triggers AuthProvider to re-read
        window.dispatchEvent(new StorageEvent("storage", { key: localStorageAuthKey }));
