...
export default function NotificationSystem() {
  ...
  const startBackgroundNotifications = () => {
    ...
    setStatus(`Background notifications started (every ${notificationInterval} seconds)`);
  ...
  const sendMultipleNotifications = () => {
    ...
    messages.forEach((msg, index) => {
      setTimeout(() => {
        sendNotification(msg.title, msg.body, `multi-${Date.now()}-${index}`);
      }, index * 1000);
    });
  };
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <div className="space-y-6">
            <div className="bg-gray-50 rounded-lg p-4">
              <h2 className="font-semibold text-gray-700 mb-2">Permission Status</h2>
              <div className="flex items-center gap-2">
                {permission === 'granted' ? (
                  ...
                  <span className="text-gray-600 font-medium">Not Requested</span>
                )}
              </div>
            </div>
            ...
            {permission === 'granted' && (
              <div className="space-y-3">
                <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-4 border border-green-200">
                  ...
                  {!isBackgroundRunning ? (
                    <button
                      onClick={startBackgroundNotifications}
                    ...
                    <button
                      onClick={stopBackgroundNotifications}
                  ...
                  {isBackgroundRunning && (
                    <p className="text-xs text-green-700 mt-2 text-center">
                      Running in background (works even in inactive tabs)
                    </p>
                  )}
                </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <ul className="space-y-2 text-sm text-gray-600">
                <li className="flex items-start gap-2">
                  ...
                  <span><strong>Background Running:</strong> Uses Web Worker to run even in inactive tabs</span>
                </li>
                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" />
                  <span><strong>Sound:</strong> Plays notification beep</span>
                </li>
                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" />
                  <span><strong>Vibration:</strong> Device vibrates on supported devices</span>
                </li>
                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" />
                  <span><strong>Permanent:</strong> Requires user interaction to dismiss</span>
                </li>
                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" />
                  <span><strong>No Grouping:</strong> Each notification has unique tag</span>
                </li>
              </ul>
            </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
              ...
              <p className="text-amber-700 text-sm mb-2">
                Notifications require browser permission and work best on desktop browsers. 
                Mobile browsers may have limitations. The notification will stay visible 
                until you interact with it.
              </p>
              <p className="text-amber-700 text-sm">
                <strong>Background Mode:</strong> Uses Web Workers to continue running even when 
                the tab is inactive or minimized. You can switch to other tabs and still receive 
                notifications at the set interval.
              </p>
            </div>
          </div>
        </div>
